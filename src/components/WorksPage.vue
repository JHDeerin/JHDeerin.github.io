...
<div id="works" class="subpage">
    ...
    <article class="display-grid">
        ...
        <sample-work-item
            ...
            img-url="/assets/img/galaxy_sim_ultramin.gif"
            github-url="https://github.com/JHDeerin/PHYS3266_NBodyGalaxy">
        </sample-work-item>
        <sample-work-item
    ...
    <footer class="header-box">
        <h2>...and the rest can be found <a href="">on GitHub</a></h2>
    </footer>
</div>
</template>

<script>
import SampleWorkItem from './SampleWorkItem.vue'

export default {
  components: {
    'sample-work-item': SampleWorkItem
  },
  mounted() {
        ...
        function updatePointConnections() {
            for (let i = 0; i < backgroundPoints.length; i++) {
                ...
                for (let j = i+1; j < backgroundPoints.length; j++) {
                    ...
                    if (Math.abs(pt1.gridBoxX - pt2.gridBoxX) > 1
        ...
        function getLineInfo(displayPts) {
            ...
            for (let displayPt1 of displayPts) {
                for (let connection of displayPt1.connections) {
                    let lineColor = `rgba(120, 128, 140, ${(0.6 * (1.0 - (connection.dist/minDistance))).toFixed(2)})`;
    ...
    let canvasVisibleCheck = new IntersectionObserver(
        (entries) => {
